...
function Shirt() {
  const [ShirtProduct, setShirtProduct] = useState([]);
  const [search, setSearch] = useState("");
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:4000/products?category_like=ao"
        );
        setShirtProduct(response.data);
      } catch (error) {
  ...
  const fillterProducts = ShirtProduct.filter((item) =>
  ...
  return (
    <div className="w-full flex justify-center items-center flex-col px-3 gap-5 mt-5 ">
      ...
      <h2 className="text-4xl font-bold">Shirt Product</h2>
          ...
          <div className="">
            ...
            <button onClick={()=>{dispatch(addtoCart(item)); navigate("/cart"); window.location.reload(); window.scrollTo(0, 0)}} className='bg-red-500 text-white font-bold w-40 h-10 justify-self-center top-5 bottom-2'>Buy now</button>
              <button
                ...
                className="bg-red-500 text-white font-bold w-40 h-10 justify-self-center top-5 bottom-2"
              >
                Add to cart
              </button>
            </div>
