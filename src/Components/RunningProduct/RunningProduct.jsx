...
import running_banner from "../../Assets/running-banner.png"
import { useDispatch } from 'react-redux';
import { addtoCart } from '../../Week5/Redux/slice/cartSlice';
import { useNavigate } from 'react-router-dom';
function RunningProduct() {
  const [runningProduct,setRunningProduct] = useState([]);
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:4000/products");
        setRunningProduct(response.data.slice(5,10)); 
      } catch (error) {
  ...
  return (
    <div className='w-full flex justify-center items-center flex-col px-3 gap-5 mt-5 '>
        <img src={running_banner} alt='runningBanner'></img>
        <h2 className='text-4xl font-bold'>Running collections </h2>
      <div className="w-full max-w-7xl grid grid-cols-5 gap-4">
      {runningProduct.map((item, i) => (
        <div className=''>
          <Item 
            key={i} 
            name = {item.name}
